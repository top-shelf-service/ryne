...
type AuthedRequest = {
  uid: string;
  user?: { uid: string; email?: string | null } & Record<string, any>;
  body: any;
  params: any;
  query: any;
};
...
export function buildOrgRouter(verifyFirebaseToken: any) {
  const r = Router();

  // POST /api/orgs — create an org; caller becomes admin/owner
  r.post('/', verifyFirebaseToken, async (req, res) => {
    try {
      const { uid, user } = req as unknown as AuthedRequest;
  ...
  r.post('/join', verifyFirebaseToken, async (req, res) => {
    try {
      ...
      const members = (memSnap.exists ? memSnap.data() : null) as
        | { owners?: string[]; managers?: string[]; staff?: string[] }
        | null;

      if (!members) return res.status(403).json({ error: 'no_membership_config' });
      ...
      if (inOwners) role = 'admin';
      else if (inManagers) role = 'manager';
      else if (inStaff) role = 'staff';
  ...
  r.get('/me', verifyFirebaseToken, async (req, res) => {
    try {
      ...
      if (!meSnap.exists) return res.status(404).json({ error: 'user_not_initialized' });

      const me = meSnap.data() as any;
      const { orgId, role } = me || {};
      if (!orgId) return res.status(404).json({ error: 'no_org' });
