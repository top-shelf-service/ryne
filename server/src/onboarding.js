...
export function onboardingRouter(verifyFirebaseToken) {
    const r = express.Router();
    // Create organization and make caller admin + active member
    r.post('/orgs', verifyFirebaseToken, async (req, res) => {
        const uid = req.uid;
        const { name } = req.body || {};
        if (!name)
    ...
    r.post('/orgs/:orgId/invites', verifyFirebaseToken, async (req, res) => {
        ...
        const cRole = caller.get('role');
        const cStatus = caller.get('status');
    ...
    r.post('/orgs/:orgId/invites/accept', verifyFirebaseToken, async (req, res) => {
        const { orgId } = req.params;
